    ...
    public List<String> findFiles(final String namePattern, boolean exact, String parent) {
        if (exact) {
            String[] all = NPath.of(parent).stream()
                    .filter(NPredicate.of((NPath x) -> namePattern.equals(x.getName())).redescribe(NDescribables.ofDesc("name='" + namePattern + "'")))
                    .map(NFunction.of(NPath::toString).redescribe(NDescribables.ofDesc("toString"))).toArray(String[]::new);
            return Arrays.asList(all);
        } else {
            final Pattern o = Pattern.compile(namePattern);
            String[] all = NPath.of(parent).stream()
                    .filter(NPredicate.of((NPath x) -> o.matcher(x.getName()).matches()).redescribe(NDescribables.ofDesc("name~~'" + namePattern + "'")))
                    .map(NFunction.of(NPath::toString).redescribe(NDescribables.ofDesc("toString"))).toArray(String[]::new);
            return Arrays.asList(all);
        }
    }
