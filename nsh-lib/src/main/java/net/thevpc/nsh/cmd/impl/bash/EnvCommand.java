...
public class EnvCommand extends NshBuiltinDefault {
    ...
    protected boolean nextOption(NArg arg, NCmdLine cmdLine, NshExecutionContext context) {
        ...
        switch (options.readStatus) {
            case 0: {
                switch (a.key()) {
                    case "--sort": {
                        return cmdLine.matcher().whenAny().asFlag((v) -> options.sort = v.booleanValue()).anyMatch();
                    }
                    case "--external":
                    case "--spawn":
                    case "-x": {
                        return cmdLine.matcher().whenAny().asTrueFlag((v) -> options.executionType = NExecutionType.SPAWN).anyMatch();
                    }
                    case "--embedded":
                    case "-b": {
                        return cmdLine.matcher().whenAny().asTrueFlag((v) -> options.executionType = NExecutionType.EMBEDDED).anyMatch();
                    }
                    case "--system": {
                        return cmdLine.matcher().whenAny().asTrueFlag((v) -> options.executionType = NExecutionType.SYSTEM).anyMatch();
                    }
                    case "--current-user": {
                        return cmdLine.matcher().whenAny().asTrueFlag((v) -> options.runAs = NRunAs.currentUser()).anyMatch();
                    }
                    case "--as-root": {
                        return cmdLine.matcher().whenAny().asTrueFlag((v) -> options.runAs = NRunAs.root()).anyMatch();
                    }
                    case "--sudo": {
                        return cmdLine.matcher().whenAny().asTrueFlag((v) -> options.runAs = NRunAs.sudo()).anyMatch();
                    }
                    case "--as-user": {
                        return cmdLine.matcher().whenAny().asEntry((v) -> options.runAs = NRunAs.user(v.stringValue())).anyMatch();
                    }
                    case "-C":
                    case "--chdir": {
                        return cmdLine.matcher().whenAny().asEntry((v) -> options.dir = v.stringValue()).anyMatch();
                    }
                    case "-u":
                    case "--unset": {
                        return cmdLine.matcher().whenAny().asEntry((v) -> options.unsetVers.add(v.stringValue())).anyMatch();
                    }
                    case "-i":
                    case "--ignore-environment": {
                        cmdLine.matcher().whenAny().asFlag((v) -> options.ignoreEnvironment = v.booleanValue()).anyMatch();
                    }
                    case "-": {
                        cmdLine.skip();
    ...
    protected void main(NCmdLine cmdLine, NshExecutionContext context) {
        ...
        if (options.command.isEmpty()) {
            ...
            final NExec e = NExec.of().command(options.command)
                    .env(env)
                    .failFast(true);
