...
public class BaseNameCommand extends NshBuiltinDefault {

    public BaseNameCommand() {
        super("basename", Options.class);
    }
    ...
    protected boolean nextOption(NArg arg, NCmdLine cmdLine, NshExecutionContext context) {
        ...
        switch (a.key()) {
            ...
            case "--all":
            case "--multi": {
                return cmdLine.matcher().whenAny().asFlag((v) -> options.multi = v.booleanValue()).anyMatch();
            }
            case "-s":
            case "--suffix": {
