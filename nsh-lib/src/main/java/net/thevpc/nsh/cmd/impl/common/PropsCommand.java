...
public class PropsCommand extends NshBuiltinDefault {
        ...
        if (cmdLine.next("get").isPresent()) {
            ...
            while (cmdLine.hasNext()) {
                if (cmdLine.next("--xml").isPresent()) {
                    ...
                    o.sourceFile = cmdLine.nextNonOption("file", NArgValueComplete.ofFlags(NArgCompleteFlag.FILENAMES)).flatMap(NArg::asString).get();
            ...
            while (cmdLine.hasNext()) {
                if (cmdLine.next("--comments").isPresent()) {
                    ...
                    o.targetFile = cmdLine.nextNonOption("file", NArgValueComplete.ofFlags(NArgCompleteFlag.FILENAMES)).flatMap(NArg::asString).get();
    ...
    private Map<String, String> readProperties(Options o, NshExecutionContext context) {
        ...
        NPath filePath = ShellHelper.xfileOf(sourceFile, context.getDirectory());
    ...
    private void storeProperties(Map<String, String> p, Options o, NshExecutionContext context) throws IOException {
        ...
        if (console) {
            ...
            NPath filePath = ShellHelper.xfileOf(targetFile, context.getDirectory());
