...
public class EnableCommand extends NshBuiltinDefault {
    ...
    protected boolean nextOption(NArg arg, NCmdLine cmdLine, NshExecutionContext context) {
        ...
        if (a.isOption()) {
            ...
            switch(a.key()) {
                case "-a": {
                    return cmdLine.matcher().whenAny().asFlag((v) -> options.a = v.booleanValue()).anyMatch();
                }
                case "-d": {
                    return cmdLine.matcher().whenAny().asFlag((v) -> options.d = v.booleanValue()).anyMatch();
                }
                case "-n": {
                    return cmdLine.matcher().whenAny().asFlag((v) -> options.n = v.booleanValue()).anyMatch();
                }
                case "-p": {
                    return cmdLine.matcher().whenAny().asFlag((v) -> options.p = v.booleanValue()).anyMatch();
                }
                case "-s": {
                    return cmdLine.matcher().whenAny().asFlag((v) -> options.s = v.booleanValue()).anyMatch();
                }
                case "-f": {
                    return cmdLine.matcher().whenAny().asEntry((v) -> options.file = v.stringValue()).anyMatch();
                }
            }
        } else {
