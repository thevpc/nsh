        ...
        in = new CloseableThread("pw-in-" + info.cmdarray[0]) {
            ...
            public void run() {
                try {
                    ...
                    int len = 0;
                    try {
                        if (in instanceof NshNonBlockingInputStream) {
                            NshNonBlockingInputStream ine = (NshNonBlockingInputStream) in;
                            while (!stopped && ine.hasMoreBytes()) {
                                ...
                                } else if (((len = ((NshNonBlockingInputStream) in).readNonBlocking(buffer, 0, buffer.length, 3000)) > 0)) {
                                    out.write(buffer, 0, len);
                                }else{
                                    break;
                                }
