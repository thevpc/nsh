...
//        c.setVerbose(isVerbose());
    ...
    @Override
    public List<NshAutoCompleteCandidate> resolveAutoCompleteCandidates(String commandName, List<String> autoCompleteWords, String autoCompleteLine, NArgCompletePosition wordPos) {
                        ...
                        for (String s : rr.split("\n")) {
                            ...
                            if (s.length() > 0) {
                                if (s.startsWith(NConstants.Apps.AUTO_COMPLETE_CANDIDATE_PREFIX)) {
                                    ...
                                    String display = null;
                                    if (args.hasNext()) {
                                        value = args.next().get().image();
                                        if (args.hasNext()) {
                                            display = args.next().get().image();
                                        }
                                    }
                                    if (value != null) {
                                        if (display == null) {
