 ...
 */
public abstract class AbstractNshContext implements NshContext {
...
//        ));
    ...
    public NshExecutionContext createCommandContext(NshBuiltin command) {
        DefaultNshExecutionContext c = new DefaultNshExecutionContext(this, command);
...
//        c.setVerbose(isVerbose());
    ...
    public List<NshAutoCompleteCandidate> resolveAutoCompleteCandidates(String commandName, List<String> autoCompleteWords, int wordIndex, String autoCompleteLine) {
        ...
        if (command != null) {
            ...
            if (nutsIds.size() == 1) {
                ...
                String nuts_autocomplete_support = NStringUtils.trim(d.getPropertyValue("nuts.autocomplete").flatMap(NLiteral::asStringValue).get());
                if (d.isApplication()
                        ...
                        for (String s : rr.split("\n")) {
                            ...
                            if (s.length() > 0) {
                                if (s.startsWith(NConstants.Apps.AUTO_COMPLETE_CANDIDATE_PREFIX)) {
                                    ...
                                    String display = null;
                                    if (args.hasNext()) {
                                        value = args.next().flatMap(NLiteral::asStringValue).get();
                                        if (args.hasNext()) {
                                            display = args.next().flatMap(NLiteral::asStringValue).get();
                                        }
                                    }
                                    if (value != null) {
                                        if (display == null) {
    ...
    @Override
    public NshContext setSession(NSession session) {
