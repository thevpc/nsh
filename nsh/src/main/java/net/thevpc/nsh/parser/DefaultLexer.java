...
public class DefaultLexer extends AbstractLexer {
    private final NshParser nshParser;
    ...
    public DefaultLexer(NshParser nshParser) {
        this.nshParser = nshParser;
    }

    public Token continueReadDollarWord() {
        StrReader reader = nshParser.strReader();
    ...
    public Token processContext(String prefix, Context t) {
        ...
        ctx.push(t);
        StringBuilder sb=new StringBuilder();
        while (nshParser.lexer().ctx.peek() != c) {
            Token tt = nshParser.lexer().nextToken(before);
            if (tt != null) {
                all.add(tt);
